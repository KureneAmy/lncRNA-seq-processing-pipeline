...
cat("\n")
arguments <- parse_args(OptionParser(usage = "%prog [options] <ERCC_FEATURECOUNTS_FILE> <GENE_FEATURECOUNTS_FILE> <ERCC_CONCENTRATION_FILE> <BAM_READ_COUNT>",
                                     option_list=list(
                                       ...
                                       make_option(c("-o","--output_prefix"), default = "", type = "character", help = "Path prefix to save the analysis output."))), 
...
ercc_conc = arguments$args[3]
read_count =  as.numeric(arguments$args[4])
out_name = opt$output_prefix
...
cat("8. The output prefix you set to save the plot and table is", opt$output_prefix, "\n")
cat("\n")
...
if (!file.exists(ercc_conc)) stop("File ERCC92_conc.txt does not exist. Check that the file is in the proper directory. You can download this file from https://www.thermofisher.com/order/catalog/product/4456740#/4456740. \n")
ercc_conc <- read_tsv(file=ercc_conc, col_names = TRUE)
ercc_conc <- ercc_conc %>% 
  select(1:4) %>% 
...
plot_name = paste(out_name, "ERCC_standard_curve.pdf", sep="_")
